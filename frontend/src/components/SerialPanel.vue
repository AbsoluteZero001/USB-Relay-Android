...
const scanButtonTooltip = computed(() => {
  if (props.platform === "android") {
    return "重新扫描 Android USB Host 设备并请求授权";
  }
  if (props.platform === "electron") {
    return "重新扫描电脑上的串口设备";
  }
  return "选择并授权浏览器可使用的串口设备";
});
...
const stateLabel = computed(() => {
  ...
  if (props.connectionState === "waiting_permission") {
    return "等待 USB 授权";
  }
  ...
  <section class="panel serial-panel">
    ...
    <dl class="device-meta">
      ...
      <div>
        ...
        <dd>
          {{
            detectedPortInfo?.vendorId && detectedPortInfo?.productId
              ? `${detectedPortInfo.vendorId} / ${detectedPortInfo.productId}`
              : "—"
          }}
        </dd>
