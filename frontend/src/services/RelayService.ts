...
export class RelayService {
  ...
  private async execute(
    ...
    try {
      ...
      this.relayState = "unknown";
      appLogger.error("继电器指令发送失败", mapped.message);
      throw mapped;
    }
    ...
    appLogger.success(
      `Relay ${channel} ${action}`,
      `${commandText} · ${this.adapter.name}`,
    );
    return {
      success: true,
      message: detail,
