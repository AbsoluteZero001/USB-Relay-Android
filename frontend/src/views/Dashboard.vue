...
const runtimePlatform: SerialRuntime = getSerialRuntime();

const PORT_POLL_INTERVAL_MS =
  runtimePlatform === "android" ? 1500 : 500;
...
const runtimeSubtitle = computed(() => {
  if (runtimePlatform === "android") {
    return "Android USB Host · 本机直连 CH340";
  }
  if (runtimePlatform === "electron") {
    return "Windows / Electron · Node SerialPort";
  }
  return "浏览器 Web Serial · 无需独立后端服务";
});
...
function applySerialStatus(next: SerialStatus): void {
  ...
  } else if (next.state === "waiting_permission") {
    connectionState.value = "waiting_permission";
    connectionMessage.value = "等待 Android USB 授权";
...
async function autoConnectDetectedPort(port: string): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      (getApiErrorCode(error)?.startsWith("USB_PERMISSION") ?? false)
    ) {
      // Do not loop the Android system permission dialog until the user
      // explicitly scans again.
...
async function connect(): Promise<void> {
  if (operationInProgress.value || !selectedPort.value) {
    if (!selectedPort.value) {
      ElMessage({
        message: "没有识别到串口设备，请确认 USB 继电器已插入并安装驱动。",
        type: "error",
        customClass: "relay-connect-error",
        grouping: false,
        duration: 3000,
      });
    }
  ...
  <div class="dashboard-shell">
    <header class="topbar">
      <div class="topbar-title">
        ...
        <div>
          ...
          <h1>USB 继电器控制台</h1>
          <p class="topbar-subtitle">{{ runtimeSubtitle }}</p>
        </div>
      </div>
    ...
    <section v-if="!webSerialSupported" class="error-banner" role="alert">
      ...
      <div>
        ...
        <p>
          <template v-if="runtimePlatform === 'android'">
            当前 Android 设备未提供 USB Host 能力或原生串口插件未加载。
          </template>
          <template v-else>
