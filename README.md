...
USB Relay Console 是一个基于 Vue 3、TypeScript 和 Capacitor 的 USB 继电器控制台，同时提供：

- **Windows 桌面版**：通过 Electron 主进程和 Node SerialPort 访问串口。
- **浏览器版**：通过 Chromium Web Serial API 访问串口。
- **Android APK**：通过 Capacitor、Kotlin 和 Android USB Host API 直接访问 USB 串口。
- **无需独立后端服务**：应用逻辑和串口通信均在本地运行。

项目已在 Windows 环境使用 CH340 和 LCUS-1 单路 USB 继电器完成实机验证。Android 代码与 APK 构建已完成验证，真实 USB OTG 硬件测试仍需在目标手机或平板上执行。
...
- 自动扫描串口并识别 CH340 设备
- 支持自动连接和异常断开后的自动重连
- Android 原生 USB Host 权限申请、串口读写和 USB 插拔监听
- 支持自定义 ON/OFF HEX 指令
...
Android 版使用 Capacitor 加载同一套 Vue 页面，通过本地 Kotlin 插件调用 Android USB Host API，并由 `usb-serial-for-android` 提供 CH340 等常见 USB 串口驱动。
...
Android 设备必须支持 USB Host / USB OTG。应用首次连接设备时会显示 Android 系统 USB 授权框。
...
```text
             ...
             Electron           Web          Android
                 |              |              |
        Node SerialPort   Web Serial API   Capacitor Plugin
                                               |
                                        Android USB Host
                                               |
...
- Android 原生层只向 WebView 暴露受限的 USB 串口插件接口
...
- Windows 10/11
- Node.js 20 或更高版本（Android 版当前使用 Capacitor 8）
- npm 10 或更高版本
- CH340 驱动
- Chrome 或 Edge 89+（仅浏览器版需要）
- JDK 21、Android SDK Platform 36（仅 Android 版需要）

安装依赖：

```powershell
cd D:\GitHub\USB-Relay-Android\frontend
npm ci
```
...
| `npm run android:open` | 使用 Android Studio 打开 `android/` |
...
- `src/services/serial/AndroidSerialAdapter.ts`：Android Capacitor 串口适配器。
- `src/services/serial/UsbRelayPlugin.ts`：Capacitor 原生插件 TypeScript 接口。
- `electron/main/serial-service.ts`：桌面版 Node SerialPort 服务。
- `electron/main/config-service.ts`：桌面版配置持久化。
- `android/.../UsbRelayPlugin.kt`：Android USB Host、权限、读写和插拔事件实现。
...
首次连接时需要允许 Android 系统的 USB 设备访问提示。应用进入后台时保持当前连接；应用销毁时关闭 `UsbSerialPort`、`UsbDeviceConnection` 和广播接收器。
...
| `USB_NOT_SUPPORTED` | Android 设备不支持 USB Host | 更换支持 USB OTG/Host 的设备 |
| `USB_DEVICE_NOT_FOUND` | 未发现 USB 串口设备 | 检查 OTG 连接和 CH340 |
| `USB_PERMISSION_DENIED` | 用户拒绝或超时未授权 | 重新扫描并允许 USB 访问 |
| `USB_PERMISSION_REQUIRED` | 尚未获得 Android USB 权限 | 重新连接并授权 |
| `USB_OPEN_FAILED` | Android 无法打开 USB 设备 | 重新插拔后重试 |
| `SERIAL_DRIVER_NOT_FOUND` | 没有匹配的 USB 串口驱动 | 确认设备支持或扩展 Prober |
| `SERIAL_DEVICE_DISCONNECTED` | USB 设备已拔出 | 重新插入并连接 |
| `INVALID_SERIAL_CONFIG` | 串口参数不受设备支持 | 使用 `9600/8/N/1` |
...
- LCUS-1 的真实硬件状态无法回读。
- Android 已完成构建验证，但尚未在真实 Android USB OTG 硬件上执行控制测试。
- 当前 Windows 发布包仅提供 Windows x64。
- 应用和安装包尚未配置代码签名。
- 当前使用默认应用图标。
