...
- 接线端子连接牢固。
- 测试环境具备过流、短路和断电保护。
- 不将本项目直接用于医疗、工业安全、消防或其他安全关键系统。

应用只提供软件控制，不替代硬件保护、电气隔离或人工安全措施。

## 已知限制

- 仅对 CH340 + LCUS-1 完成了 Windows 实机协议验证。
- Android APK 已完成构建验证，但真实 USB OTG 硬件测试待执行。
- LCUS-1 暂无已验证的状态回读协议。
- 当前未实现 Android 后台常驻、开机启动或 Foreground Service。
- Android Release APK/AAB 尚未配置正式签名。
...
当前仓库尚未添加 `LICENSE`。公开发布前必须明确许可证，例如 MIT、Apache-2.0 或 GPL-3.0。没有许可证时，其他人默认没有复制、修改和再发布代码的权利。
...
发布签名密码和密钥别名应放入 GitHub Actions Secrets，不应写进仓库。

### 3. 区分 Debug 与 Release

`app-debug.apk` 只适合内部调试，不应作为正式发行包。正式发布应提供：
...
发布前需要配置 Release 签名、版本号和混淆策略。

### 4. 使用 GitHub Releases

建议按版本创建 Git Tag 和 GitHub Release，例如：

```text
v0.5.0
```

Release 中附上签名 APK、可选 AAB、SHA256 校验值和更新说明。不要只把 APK 提交到 Git 仓库。

### 5. 真实验证状态必须诚实

Android 当前只能声明“构建验证通过”。在真实手机、USB OTG、CH340 和 LCUS-1 上完成 ON/OFF 测试后，再改为“Android 实机验证通过”。
...
Android 清单包含 USB Host 能力和 Capacitor 模板的 `INTERNET` 权限。串口控制本身完全在本机执行，不依赖远程后端。如果后续完全不使用网络能力，可以评估移除 `INTERNET` 权限并在 README 中明确说明。
...
本项目按“现状”提供。作者不对设备损坏、负载失控、数据丢失、人身伤害或任何间接损失承担责任。使用者必须自行验证接线、电压、隔离和故障保护方案。
